################ Henri Lahousse ##################
# object tracking with tello drone, follows certain color
# not originally my code, I just adjusted some stuff and made it up to date
# 05/05/2022

# libraries
from djitellopy import Tello       # for tello drone   
import cv2                         # image processing         
import numpy as np                 # array's and matrices            


######################################################################
width = 640   # witdh of picture
height = 480  # height of picture
deadZone = 100
######################################################################

startCounter = 0

# verbinden met tello drone
me = Tello()
me.connect()

# instellen velocities
...
print(me.get_battery())  # battery percentage, a lot of other possibilities like speed, barometer, height,...

me.streamoff()
me.streamon()  # start live feed camera
########################
frameWidth = width
frameHeight = height
...
while True:
    ...
    if startCounter == 0:
       me.takeoff()
       # add starting possition if necessary
       startCounter = 1
    ...
    if dir == 1:
       ...
       #me.up_down_velocity = 60  # if you want the drone to go down when the lower field detects object
       me.for_back_velocity = 60  # drone goes back when lower field detects object
    elif dir == 4:
       #me.up_down_velocity = -60 # if you want the drone to go up when the upper field detects object
       me.for_back_velocity = -60 # if you want the drone to go forward when the upper field detects object
    else:
